...
- Add an R2 CORS policy that allows browser uploads from your app domain.
  ...
  - Exposed headers: none required for upload, but ensure the bucket accepts the browser origin and request method
  - Max age: 600

  The browser upload flow uses a presigned PUT URL, so the R2 bucket must allow cross-origin PUT requests from your app origin with the `Content-Type` header.
