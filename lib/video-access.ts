...
export function canAccessVideo(video: AccessVideo, auth: Pick<AuthTokenPayload, 'role' | 'sub'> | null | undefined) {
  return video.status === 'APPROVED' || canPreviewVideo(video, auth);
}
...
export function isPlayableVideo(video: Pick<AccessVideo, 'videoType' | 'seriesId' | 'r2Key'>) {
  return !isSeriesContainer(video) && Boolean(video.r2Key);
}
