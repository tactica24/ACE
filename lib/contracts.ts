...
export function generateContract(args: {
  ...
  return [
    ...
    `Producer: ${args.creatorName}`,
    `Title: ${args.videoTitle}`,
    `Rights Tier: ${tierLabel}`,
    `Revenue Split: ${payoutPercent}% Producer / ${platformPercent}% ACE`,
    `Grant of Rights: The Producer grants ACE the right to distribute the title within ACE channels, including offline encrypted distribution via .ace files.`,
    ...
    `Payments: Producer payouts settle weekly to the registered payout account.`,
