...
export function alignNairaToCreditValue(amountNaira: number, minimum = CREDIT_UNIT_VALUE_NAIRA) {
  if (!Number.isFinite(amountNaira) || amountNaira <= 0) {
    return minimum;
  }

  return Math.max(minimum, Math.round(amountNaira / CREDIT_UNIT_VALUE_NAIRA) * CREDIT_UNIT_VALUE_NAIRA);
}
