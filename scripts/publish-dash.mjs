...
const inputRoot = args.input || path.resolve('storage', 'dash');
const localDir = path.join(inputRoot, videoId || '');

if (!videoId) {
  console.error('Usage: node scripts/publish-dash.mjs --videoId=abc123 [--input=storage/dash]');
  process.exit(1);
}
...
for (const filePath of files) {
  ...
  const key = `streams/${videoId}/dash/${relativePath}`;
