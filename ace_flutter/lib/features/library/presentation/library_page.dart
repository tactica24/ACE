...
enum _LibraryTab { available, offline }
...
class _LibraryPageState extends ConsumerState<LibraryPage> {
  _LibraryTab _activeTab = _LibraryTab.available;
      ...
      body: Column(
        ...
        children: [
          Wrap(
            ...
            children: [
              ChoiceChip(
                selected: _activeTab == _LibraryTab.available,
                label: const Text('Available'),
                onSelected: (_) {
                  setState(() {
                    _activeTab = _LibraryTab.available;
                  });
                },
              ),
              ChoiceChip(
                selected: _activeTab == _LibraryTab.offline,
                label: const Text('Offline'),
                onSelected: (_) {
                  setState(() {
                    _activeTab = _LibraryTab.offline;
                  });
                },
              ),
            ],
          ...
          if (_activeTab == _LibraryTab.available)
            _buildCloudLibrary()
          else
            _buildOfflineDownloads(),
        ],
      ),
    );
  }
        ...
        return Column(
          ...
          children: [
            ...
            ...downloads.map((download) => _DownloadedCard(
                  download: download,
                  onPlay: () =>
                      context.push('/offline-player', extra: download),
                  onRemove: () async {
                    await ref
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 10),
          Text(
            'Offline titles are linked to your signed-in profile on this device.',
            style: Theme.of(context)
                .textTheme
                .bodyMedium
                ?.copyWith(color: AppTheme.textMuted),
