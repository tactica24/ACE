...
final offlineDownloadsProvider = FutureProvider.family<List<DownloadedTitle>, String>(
  (ref, principalId) async {
    return ref.watch(offlineDownloadRepositoryProvider).listDownloads(principalId);
  },
);

final downloadedTitleProvider =
    FutureProvider.family<DownloadedTitle?, ({String principalId, String titleId})>(
  (ref, args) async {
    return ref
...
class OfflineDownloadRepository {
  ...
  Future<DownloadedTitle> downloadTitle({
    ...
    if (!isSignedIn || !detail.access.hasAccess) {
      throw Exception('Only unlocked titles can be downloaded for offline playback.');
    }
    ...
    if (streamed.statusCode >= 400) {
      throw ApiException('Unable to download this title right now.', statusCode: streamed.statusCode);
    }
    ...
    final targetFile = File('${directory.path}/$safeId.mp4');
  ...
  Future<List<DownloadedTitle>> _readDownloads(String principalId) async {
    ...
    return decoded
        .whereType<Map>()
        .map((item) => DownloadedTitle.fromJson(Map<String, dynamic>.from(item)))
        .toList();
  }

  Future<void> _saveDownloads(String principalId, List<DownloadedTitle> downloads) async {
    final prefs = await SharedPreferences.getInstance();
  ...
  String _downloadsPrefKey(String principalId) => '$_downloadsPrefKeyPrefix$principalId';
