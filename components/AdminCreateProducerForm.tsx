...
export default function AdminCreateProducerForm() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setMessage(`Approved producer created: ${payload.producer.displayName} (${payload.producer.creatorNumber}). Opening upload desk...`);
      window.location.href = `/admin/upload?producerId=${encodeURIComponent(payload.producer.id)}`;
    } catch (error) {
    ...
    <form className="form-section" onSubmit={handleSubmit}>
      ...
      <div className="form-actions">
        <button className="btn btn-primary" type="submit" disabled={loading}>
          {loading ? 'Creating producer...' : 'Create producer and open upload'}
        </button>
        <span className="muted">This producer becomes available immediately in the admin upload desk.</span>
      </div>
