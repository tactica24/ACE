...
export async function POST(req: NextRequest) {
  ...
  const nextState = {
    phoneVerified: field === 'phoneVerified' ? value : profile.phoneVerified,
  ...
  const updated = await prisma.creatorProfile.update({
    ...
    data: {
      [field]: value,
      verified: nextState.phoneVerified && nextState.emailVerified && nextState.ninVerified && nextState.idVerified && nextState.bankVerified
    }
  });
