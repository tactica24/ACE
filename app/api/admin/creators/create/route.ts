...
export async function POST(req: NextRequest) {
  ...
  if (!displayName || !email || !phone) {
    return NextResponse.json({ error: 'Producer name, email, and phone are required.' }, { status: 400 });
  }
  ...
  if (emailMatch && phoneMatch && emailMatch.id !== phoneMatch.id) {
    return NextResponse.json({
      error: 'The email and phone already belong to different accounts. Resolve that conflict before creating this producer.'
    }, { status: 400 });
  }

  const existingUser = emailMatch ?? phoneMatch ?? null;
  ...
  try {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
      return NextResponse.json({
        error: 'A producer account with this email, phone, or code already exists.'
      }, { status: 400 });
    }
