...
function generateLinkToken(creatorUserId: string, scope: 'upload') {
  return createCreatorAccessLinkToken({
    creatorUserId,
    scope,
    expiresIn: '365d'
  });
...
export async function POST(req: NextRequest) {
  ...
  const existingProfile = await prisma.creatorProfile.findUnique({
    ...
    select: { creatorNumber: true, emailVerified: true }
  });
  ...
  const emailVerified = existingProfile?.emailVerified || Boolean(auth.emailVerified);
   ...
   // Generate permanent access token for upload (valid for 1 year)
   const uploadToken = generateLinkToken(auth.sub, 'upload');
   const expiresAt = new Date(Date.now() + 365 * 24 * 60 * 60 * 1000);

   await prisma.creatorProfile.update({
     where: { userId: auth.sub },
     data: {
        uploadAccessToken: uploadToken,
        uploadAccessTokenExpiresAt: expiresAt
     }
   });
